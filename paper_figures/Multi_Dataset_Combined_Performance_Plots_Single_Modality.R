...
# This script is for single-modality analysis (RNA-only or ATAC-only)
...
# Output directory
output_dir <- file.path("~/scMultiPreDICT_output/paper_figures", MODALITY, GENE_SET)
...
ggsave(file.path(output_dir, "Fig2_Correlation_RNA_only_hvg.pdf"), p_raincloud, device = cairo_pdf,
  width = 12, height = 7, units = "in")
ggsave(file.path(output_dir, "Fig2_Correlation_RNA_only_hvg.png"), p_raincloud, width = 12, height = 7, units = "in", dpi = 600)
...
summary_stats <- all_data %>%
  ...
  summarise(across(all_of(metrics_present), ~ median(.x, na.rm = TRUE), .names = "Median_{.col}"), .groups = "drop")
...
if ("R2" %in% colnames(all_data)) {
  ...
  # mean labels removed for cleaner appearance
    ...
    ggsave(file.path(output_dir, "Fig2_R2_RNA_only_hvg.pdf"), p_box_r2, device = cairo_pdf,
      width = 12, height = 8, units = "in")
    ggsave(file.path(output_dir, "Fig2_R2_RNA_only_hvg.png"), p_box_r2, width = 12, height = 8, units = "in", dpi = 600)
}
...
if ("RMSE" %in% colnames(all_data)) {
  ...
  # mean labels removed for cleaner appearance
    ...
    ggsave(file.path(output_dir, "Fig2_RMSE_RNA_only_hvg.pdf"), p_box_rmse, device = cairo_pdf,
      width = 12, height = 8, units = "in")
    ggsave(file.path(output_dir, "Fig2_RMSE_RNA_only_hvg.png"), p_box_rmse, width = 12, height = 8, units = "in", dpi = 600)
}
