...
6. [High-Performance Computing (HPC) Usage](#high-performance-computing-hpc-usage)
7. [Target Gene Configuration](#target-gene-configuration)
8. [Reproducing Published Results](#reproducing-published-results)
...
- **Dimensionality Reduction and Integrated Analysis Approaches form Metcall Construction**
...
sbatch step_025_my_sample.sbatch     # GPU autoencoder training for scVI, PeakVI and MultiVI
...
# Or specify the full path in script
```

#### 2. Out of Memory
```r
# Increase memory in config.R
SLURM_RESOURCES$step_050$mem_gb <- 512
# Or use disk-backed matrices
USE_DISK_BACKED <- TRUE
```

#### 3. "Gene not found in expression matrix"
```r
