# RNA-only Pipeline - Dataset Configuration Directory
...
- The RNA-only pipeline requires outputs from the **combined pipeline** 
...
- All dataset configs should use the same `CONDA_ENV_R` and cluster settings
  (defined in `../submit_settings.sh`), but can have different data paths, 
  species, and analysis parameters.

## Example Config Differences Between Datasets

For `E7.5_rep1.config.R`:
```r
SAMPLE_NAME <- "E7.5_rep1"
SPECIES <- "mouse"
INPUT_SEURAT_SPLITS <- "/path/to/combined/output/splits/E7.5_rep1/E7.5_rep1_seurat_obj_with_splits.rds"
```

For `T_Cells.config.R`:
```r
SAMPLE_NAME <- "T_Cells"
SPECIES <- "human"
INPUT_SEURAT_SPLITS <- "/path/to/combined/output/splits/T_Cells/T_Cells_seurat_obj_with_splits.rds"
```
