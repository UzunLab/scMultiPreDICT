#!/bin/bash
# ============================================================================
# Multi-Dataset Pipeline Submission - Combined (RNA+ATAC)
# ============================================================================
#
# Submits the scMultiPreDICT combined pipeline for MULTIPLE datasets.
...
#   2. Create per-dataset configs in datasets/ (copy from R/config_template.R)
#   3. Run: ./submit_datasets.sh
#
# Usage:
#   ./submit_datasets.sh                      # Submit ALL datasets
#   ./submit_datasets.sh --dry-run            # Preview without submitting
#   ./submit_datasets.sh --datasets d1,d2     # Submit specific datasets
#   ./submit_datasets.sh --start-step 3       # Start from metacell creation
#   ./submit_datasets.sh --stop-step 4        # Stop after feature extraction
...
#   5: Linear + Random Forest Model Training
...
Options:
  ...
  --stop-step N         Stop pipeline after step N (default: 6)
...
Steps:
  ...
  6  Neural Network         Feed-forward NN with grid search

Note: If using multivi or scvi_peakvi dimensionality reduction, you need to
run the Python autoencoder training step separately before step 3. See the
README in datasets/ for details.

Examples:
  # Submit all datasets, full pipeline
  ./submit_datasets.sh

  # Dry run to preview
  ./submit_datasets.sh --dry-run

  # Submit only E7.5_rep1 starting from metacell creation
  ./submit_datasets.sh --datasets E7.5_rep1 --start-step 3

  # Run QC + splitting only for all datasets
  ./submit_datasets.sh --stop-step 2

  # Only model training (requires steps 1-4 already completed)
  ./submit_datasets.sh --start-step 5 --stop-step 6
EOF
            exit 0 ;;
        *)
...
if [ ! -d "$DATASET_DIR" ]; then
    ...
    echo "  cp R/config_template.R datasets/MY_DATASET.config.R"
...
echo "║              Combined (RNA+ATAC) Pipeline                   ║"
echo "╚══════════════════════════════════════════════════════════════╝"
echo ""
echo "  Pipeline dir:  $PIPELINE_DIR"
echo "  Partition:     $PARTITION"
echo "  Conda env (R): $CONDA_ENV_R"
