...
# - Combine into joint multimodal space with z-score using WNN
...
#   2. Run: Rscript 03a_metacell_creation_wnn.R
# ============================================================================
...
save_plot <- function(plot, filename, width = 7, height = 5, dpi = 600) {
  # Save PNG (for presentations, quick viewing)
  ggsave(paste0(filename, ".png"), plot = plot, 
         width = width, height = height, dpi = dpi)
  
 # Save PDF (for publications, vector graphics)
...
# Build LSI transformation from TRAINING ATAC data only
# - Uses FindTopFeatures(min.cutoff = "q05") 
# - Uses simple TRAIN IDF from counts: idf = log1p(n_cells / nz_per_peak)
# - Runs RunTFIDF + RunSVD (Signac)
# - Drops component 1 (depth) in BOTH embeddings and loadings
# - Returns:
#     feature_loadings: peaks × dims (comp1 removed)
#     train_scores    : cells × dims (comp1 removed)
#     idf             : TRAIN IDF per peak
#===============================================================================================
build_atac_lsi_signac <- function(seurat_obj, assay = "ATAC", ncomp = 30) {
  # Work on ATAC assay
  DefaultAssay(seurat_obj) <- assay
  
  # Ensure ATAC counts are sparse (for memory/speed)
  ...
  # 5) Extract LSI embeddings (cells × comps) and loadings (peaks × comps)
  lsi_coords_full       <- Embeddings(seurat_obj, "lsi")
  feature_loadings_full <- Loadings(seurat_obj, "lsi")
  
  # Drop first component (captures depth) in BOTH embeddings and loadings
  lsi_coords      <- lsi_coords_full[, -1, drop = FALSE]       # cells × (ncomp-1)
  feature_loadings <- feature_loadings_full[, -1, drop = FALSE] # peaks × (ncomp-1)
  
  cat(sprintf(
    "[INFO] ATAC LSI complete: %d peaks × %d cells; %d components (skipped 1st)\n",
    nrow(feature_loadings), nrow(lsi_coords), ncol(lsi_coords)
  ))
  
  list(
    seurat_obj       = seurat_obj,        # Seurat with TF-IDF + LSI (optional)
    feature_loadings = feature_loadings,  # peaks × dims (comp1 removed)
    train_scores     = lsi_coords,        # cells × dims (comp1 removed)
    n_components     = ncol(lsi_coords),
    idf              = idf_vector        # TRAIN IDF for all peaks
  )
}
...
project_atac_lsi_signac <- function(seurat_obj_split, train_fit, assay = "ATAC") {
  ...
  # TRAIN parameters
  L      <- train_fit$feature_loadings          # peaks × dims (comp1 dropped)
  idf    <- train_fit$idf                       # TRAIN IDF (all peaks)
  tpeaks <- rownames(L)                         # training peaks
  
  # Split counts (peaks × cells)
  Xc <- GetAssayData(seurat_obj_split, assay = assay, slot = "counts")
  if (!inherits(Xc, "dgCMatrix")) {
    Xc <- as(Xc, "dgCMatrix")
  }
  
  # Intersect peaks, keep TRAIN order
  ...
  if (length(common) < max(500, floor(0.5 * length(tpeaks)))) {
    stop(sprintf("Too few overlapping peaks for projection: %d/%d",
                 length(common), length(tpeaks)))
  }
  
  Xc   <- Xc[common, , drop = FALSE]       # peaks × cells
  Luse <- L[common, , drop = FALSE]        # peaks × dims
  idf  <- idf[common]                      # TRAIN IDF for these peaks
  ...
  # TF-IDF with TRAIN IDF
  TFIDF <- TF * idf                        # peaks × cells
  
  # Project into TRAIN LSI space: (cells × peaks) %*% (peaks × dims)
  emb <- as.matrix(Matrix::t(TFIDF) %*% Luse)  # cells × dims
...
# using pca from RNA and lsi from ATAC to find a common shared space
#
# Arguments:
#     seurat_split: Seurat object containing train, val and test splits cells
#     reduction.list: contains list of the pca and lsi for each split (these were only computed on train and projected onto the train and test set)
#     dims.list : contains the specified number of dimensions to use
#
# Returns:
#     wnn_scores: matrix (#cells_in_split x #dims (shared dimension))
#-----------------------------------------------------------------------------------------------
build_wnn <- function(
    seurat_obj,
    reduction.list = list("pca_custom", "lsi_custom"),
  ...
  # 1. Build WNN using the specified reductions
  ...
  # 2. Extract the WNN Graph (cells x cells)
  wnn_graph <- seurat_obj@graphs$wknn   # cells x cells
  
  # 3. Compute a low dimensional WNN embedding from the graph
  # Here I use irlba on the adjacency matrix to get the wnn_dims components
  wnn_emb <- irlba::irlba(wnn_graph, nv = 50)$u # this gives cells x wnn_dims
  
  # Make sure the rownames match cell names
  rownames(wnn_emb) <- colnames(seurat_obj)
  
  # Return a list
  list(
    seurat_obj = seurat_obj,
    wnn_emb = wnn_emb  # this is what will be passed into the build_knn_smooth function
  )
}
  

# ===============Z-score and k-NN Functions======================================================
...
#       Each column sums to 
#=================================================================================================
build_knn_smooth <- function(wnn_scores, cell_names, k_smooth = 20, seed = 1) {
  set.seed(seed)
  n <- nrow(wnn_scores)
...
smooth_expression <- function(rna_counts, atac_counts, A_smooth) {
  rna_smoothed <- rna_counts %*% A_smooth    # smoothed RNA_counts
  atac_smoothed <- atac_counts %*% A_smooth  # smoothed ATAC_counts
  ...
  list(
    rna_log1p = rna_log1p,  # normalized smoothed RNA expression
    rna_lib = rna_lib,      # RNA library size per smoothed cell
    atac_counts = atac_smoothed,  # smoothed atac raw counts
    atac_lib = atac_lib,         # ATAC library size per smoothed cell
    atac_log1p = atac_log1p   # normalized smoothed ATAC accessibility
  )
}

# ===============Build Multimodal Joint Space for TRAINING===============================================
...
# Build ATAC LSI from training data using Signac
cat("[INFO] Building ATAC LSI from training data (using Signac)...\n")
...
head(atac_scores_train)
head(rna_scores_train)

# Attach the PCA and LSI embeddings to the seurat obj
seurat_train[["pca_custom"]] <- CreateDimReducObject(
  embeddings = rna_scores_train,
  key = "PC_",
  assay = "RNA"
)

seurat_train[["lsi_custom"]] <- CreateDimReducObject(
  embeddings = atac_scores_train,
  key = "LSI_",
  assay = "ATAC"
)

# Build WNN graph for train only
wnn_train <- build_wnn(
  seurat_obj = seurat_train,
  reduction.list = list("pca_custom", "lsi_custom"),
  dims.list = list(1:30, 1:29),
  wnn_dims = 50,
  seed = seed
...
# Extract the WNN scores and seurat obj
seurat_train <- wnn_train$seurat_obj
wnn_scores_train <- wnn_train$wnn_emb

dim(wnn_scores_train)
...
A_smooth_train <- build_knn_smooth(
  wnn_scores_train,
  train_cells,
  k_smooth = min(k_neighbors, nrow(wnn_scores_train)),
  seed = seed
)

# QC: Check k-NN statistics
neighbors_per_cell <- Matrix::colSums(A_smooth_train > 0)
...
# Save training transformation parameters for val/test
train_transforms <- list(
  rna_fit = rna_fit,      # Full PCA parameters
  atac_fit = atac_fit,    # Full LSI parameters,
  pca_dims = pca_dims,
  k_neighbors = k_neighbors,
  n_variable_features = n_variable_features
)
saveRDS(train_transforms, file.path(OUTPUT_METACELLS_DIR, "train_transforms_wnn.rds"))
cat("[OK] Saved train_transforms_wnn.rds\n")
...
train_output <- list(
  ...
  wnn_scores = wnn_scores_train,
  joint_space = wnn_scores_train,
  cells = train_cells,
  split_name = "train",
...
train_transforms <- readRDS(file.path(OUTPUT_METACELLS_DIR, "train_transforms_wnn.rds"))

for (split_name in c("validation", "test")) {
  cat(sprintf("\n--- Processing %s ---\n", toupper(split_name)))
  
  # Subset to split cells
  ...
  seurat_split <- normalize_rna(seurat_split)  # Normalize before projection
  ...
  # Ensure ATAC counts are sparse
  ac <- GetAssayData(seurat_split, assay = "ATAC", layer = "counts")
  if (!inherits(ac, "dgCMatrix")) {
    seurat_split <- SetAssayData(seurat_split, assay = "ATAC", layer = "counts",
  ...
  # PROJECT into training LSI space using Signac
  cat(sprintf("[INFO] Projecting %s ATAC into TRAINING LSI space (Signac)...\n", split_name))
  ...
  # Attach the PCA and LSI embeddings to the seurat obj
  seurat_split[["pca_custom"]] <- CreateDimReducObject(
    embeddings = rna_scores_split,
    key = "PC_",
    assay = "RNA"
  )
  
  seurat_split[["lsi_custom"]] <- CreateDimReducObject(
    embeddings = atac_scores_split,
    key = "LSI_",
    assay = "ATAC"
  )
  
  # Build WNN for each split (critical to prevent data leakage)
  wnn_split <- build_wnn(
    seurat_obj = seurat_split,
    reduction.list = list("pca_custom", "lsi_custom"),
    dims.list = list(1:30, 1:29),
    wnn_dims = 50,
    seed = seed
  ...
  # Extract WNN embedding for each split
  wnn_scores_split <- wnn_split$wnn_emb
  
  cat(sprintf("Joint space: %d cells × %d dimensions\n", nrow(wnn_scores_split), ncol(wnn_scores_split)))
  ...
  A_smooth_split <- build_knn_smooth(
    wnn_scores_split,
    split_cells,
    k_smooth = min(k_neighbors, nrow(wnn_scores_split)),
    seed = seed
  )
  
  # QC: Check k-NN statistics
  neighbors_per_cell <- Matrix::colSums(A_smooth_split > 0)
  cat(sprintf("k-NN stats: mean=%.1f, min=%d, max=%d neighbors\n",
              mean(neighbors_per_cell), min(neighbors_per_cell), max(neighbors_per_cell)))
  
  # Smooth using ONLY cells from this split (critical for preventing leakage)
  cat(sprintf("[INFO] Smoothing %s expression (using only %s neighbors)...\n", split_name, split_name))
  ...
  split_output <- list(
    ...
    rna_pca = rna_scores_split,       # Projected coordinates (in training space!)
    atac_lsi = atac_scores_split,     # Projected coordinates
    wnn_scores = wnn_scores_split,        # Joint coordinates (comparable to training)
    joint_space = wnn_scores_split,
    cells = split_cells,
    split_name = split_name,
    processing_seed = seed,
    k_smooth = k_neighbors,
...
# Use split colors (defined in theme section)
...
cairo_pdf(file.path(plots_dir, "Step_050_Metacell_Report.pdf"), width = 10, height = 8)
...
cat("Key features:\n")
cat("  ✓ Training: Built WNN transformations from scratch\n")
